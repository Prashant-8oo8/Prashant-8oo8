...
const row1 = [
  ...
  { name: 'Three.js', icon: SiThreedotjs, color: '#18181b' },
...
const row2 = [
  { name: 'Next.js', icon: SiNextdotjs, color: '#18181b' },
  { name: 'JavaScript', icon: SiJavascript, color: '#F7DF1E' },
  { name: 'Redux', icon: SiRedux, color: '#764ABC' },
  { name: 'Appwrite', icon: SiAppwrite, color: '#FD366E' },
  { name: 'GSAP', icon: SiGreensock, color: '#88CE02' },
  { name: 'Vite', icon: SiVite, color: '#646CFF' },
  { name: 'GitHub', icon: SiGithub, color: '#18181b' },
    ...
    return (
      ...
      >
        {[...items, ...items].map((item, i) => (
          <div 
            key={i} 
            className="flex flex-col items-center justify-center w-16 h-16 md:w-20 md:h-20 shrink-0 bg-zinc-100 border border-zinc-200 rounded-3xl transition-all duration-300 grayscale opacity-70 hover:grayscale-0 hover:opacity-100 hover:border-[#FFD700]/50 hover:shadow-lg cursor-pointer"
          >
            <item.icon className="text-2xl md:text-3xl mb-1 md:mb-2" style={{ color: item.color }} />
            <span className="text-[10px] font-mono text-zinc-500">{item.name}</span>
          </div>
        ))}
      </div>
    );
